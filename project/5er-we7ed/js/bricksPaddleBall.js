window.onload = function () {
    ...
    createNet(context2, 100, 30)
    createNet(context1, 400, 30)
            ...
            if (bricks[r][c].status) {
                ...
                img.addEventListener('load', function () { //load because we create the image so it will take time to be onload
                    ...
                    let brickInterval = setInterval(() => {
                        ...
                        //clear drawing area (x,  y, width, height)
                        ctx.clearRect(bricks[r][c].x, bricks[r][c].y, 84, 68)
                        ctx.drawImage(img, imgCounter, 547.8, 84, 68, bricks[r][c].x, bricks[r][c].y, 84, 68)
                        imgCounter += 84
        ...
        for (let c = 0; c < r; c++) { //to create the columns
            ...
            if (bricks[r][c].status) {
                ...
                imgShark.addEventListener('load', function () { //load because we create the image so it will take time to be onload
                    ...
                    let imgCounter = 0
                    let sharkInterval1 = setInterval(() => {
                    ...
                    bricks[r][c].interval = sharkInterval1
                })
            }
        ...
        for (let c = 0; c < brick.column; c++) { //to create the columns
            ...
            if (b.status) {
                if ((ball.x + ball.radius > b.x) && (ball.x - ball.radius < b.x + brick.width) && (ball.y + ball.radius > b.y) && (ball.y - ball.radius < b.y + brick.height)) {
                    ...
                    ball.dy = -ball.dy
                    //alert('hi')
                    console.log(bricks[r][c].x, bricks[r][c].y);
...
function ballBricksCollision1(context3, brick, bricks, ball) {
